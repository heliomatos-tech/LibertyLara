<div x-data="{ open: false, selected: '{{ $selectedText ?? '' }}', value: {{ $selectedValue }}, dropdownFitsBelow: true, checkDropdownPosition() { this.dropdownFitsBelow = window.innerHeight - this.$refs.dropdown.getBoundingClientRect().bottom > this.$refs.dropdown.offsetHeight; } }" x-on:click.away="open = false" class="relative" x-ref="dropdown">
    <div x-on:click="open = !open; checkDropdownPosition()"
        class="flex items-center cursor-pointer w-full text-base leading-normal form-input text-gray-800 border border-gray-200 rounded-md shadow-sm">
